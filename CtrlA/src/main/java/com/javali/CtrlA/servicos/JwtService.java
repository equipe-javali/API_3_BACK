...
public class JwtService {
	...
	public String createToken(String email, long expiration, String secret) {
		Date now = new Date(System.currentTimeMillis());
		Date time = new Date(System.currentTimeMillis() + expiration);

		return Jwts.builder().setSubject(email).setExpiration(time).setIssuedAt(now)
	...
	public Boolean validateToken(String jwtToken, String secret, String email) {
		String subject = extractUsername(jwtToken, secret);
		return (subject.equals(email) && isNotTokenExpired(jwtToken, secret));
	}
}
